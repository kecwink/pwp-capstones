...
def average_sentence_length(raw_text):
    sentence_lengths =[]
    sentence_words=[]
    for sentences in sentences_in_text:
            sentence_words.append(sentences.split(" "))
    ...
    average_sentence_length = (total_length/(len(sentence_lengths)))
...
lily_trebuchet_string= """Hi, I'm Lily Trebuchet from East Egg, Long Island. I love cats, hiking, and curling up under a warm blanket with a book. So they gave this little questionnaire to use for our bios so lets get started. What are some of my least favorite household chores? Dishes, oh yes it's definitely the dishes, I just hate doing them, don't you? Who is your favorite actor and why? Hmm, that's a hard one, but I think recently I'll have to go with Michael B. Jordan, every bit of that man is handsome, HANDSOME! Do you remember seeing him shirtless? I can't believe what he does for the cameras! Okay okay next question, what is your perfect date? Well it starts with a nice dinner at a delicious but small restaurant, you know like one of those places where the owner is in the back and comes out to talk to you and ask you how your meal was. My favorite form of art? Another hard one, but I think I'll have to go with music, music you can feel in your whole body and it is electrifying and best of all, you can dance to it! Okay final question, let's see, What are three things you cannot live without? Well first off, my beautiful, beautiful cat Jerry, he is my heart and spirit animal. Second is pasta, definitely pasta, and the third I think is my family, I love all of them very much and they support me in everything I do. I know Jay Stacksby is a handsome man and all of us want to be the first to walk down the aisle with him, but I think he might truly be the one for me. Okay that's it for the bio, I hope you have fun watching the show! """

def average_sentence_length(raw_text):
    for sentences in raw_text:
        no_questions = raw_text.replace('?', '.')
        no_exclamations=no_questions.replace('!', '.')
        split_sentences = no_exclamations.split(' ')
...
print(average_sentence_length(lily_trebuchet_string))
...
print(average_sentence_length(myrtle_beech_intro))
...
print(average_sentence_length(gregg_t_fishy_intro))
...
print(average_sentence_length(murder_note))
...
class TextSample:
    def __init__(self, text, author, prepared_text):
        self.raw_text = text
        self.author = author
        self.average_sentence_length = average_sentence_length(self.raw_text)
        self.prepared_text = prepare_text(self.prepared_text)
    def __repr__(self):
        return self.author + " wrote " + self.raw_text + " which has an average sentence length of "+ (str(self.average_sentence_length))
...
murderer_sample = TextSample("murder note", "murderer")
...
lily_sample = TextSample("Lily's note", "Lily Trebuchet")
print(lily_sample)
...
myrtle_sample = TextSample("Myrtle's note", "Myrtle Beech'")
print(myrtle_sample)
...
gregg_sample = TextSample("Gregg T Fishy's note", "Gregg T Fishy")
print(gregg_sample)
...
def build_frequency_table (corpus):
  corpus = prepared_text
  occurances = []
  for word in clean_murder_note:
        occurances.append(prepared_text.count(prepared_text[0:-1]))
          #occurances += 1
  return occurances
  zip(corpus, occurances)
  frequency_table = {key:value for key, value in zip(corpus, occurances)}
